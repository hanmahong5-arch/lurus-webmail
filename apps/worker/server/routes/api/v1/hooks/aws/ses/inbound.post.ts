...
export default defineEventHandler(async (event) => {
	try {
		...
		// Parse SNS payload with error handling
		let sns: Record<string, unknown>;
		try {
			sns = JSON.parse(raw as string);
		} catch (parseError) {
			console.error("[Webhook] Failed to parse SNS payload:", parseError);
		...
		if (sns?.Type === "Notification" && sns?.Message) {
			...
			try {
				...
				console.error("[Webhook] Failed to parse SNS Message:", parseError);
				return { ok: false, error: "Invalid SNS Message JSON" };
			}
			const rec = msg?.Records?.[0] as Record<string, unknown> | undefined;
			if (!rec || rec.eventSource !== "aws:s3") {
				console.log("[Webhook] Non-S3 notification, ignoring.");
				return { ok: true };
			}

			const s3Record = rec?.s3 as { bucket?: { name?: string }; object?: { key?: string; size?: number } } | undefined;
			const bucket: string = s3Record?.bucket?.name ?? "";
			const key: string = decodeURIComponent(s3Record?.object?.key || "");
			const size: number = s3Record?.object?.size ?? 0;

			if (!bucket || !key) {
				console.error("[Webhook] Missing S3 bucket or key in record");
				return { ok: false, error: "Missing S3 bucket or key" };
			}
			...
			if (secrets?.vault?.decrypted_secret) {
				try {
					vaultValues = JSON.parse(secrets.vault.decrypted_secret);
				} catch (parseError) {
					console.error("[Webhook] Failed to parse vault secret:", parseError);
					return { ok: false, error: "Invalid vault secret format" };
				}
			}
