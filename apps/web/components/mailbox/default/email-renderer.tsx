...
import QuickReplyBox from "@/components/mailbox/default/quick-reply-box";
  ...
  "
			...
			{threadIndex === numberOfMessages - 1 && !showEditor && (
				<div className="mt-4">
					...
					<QuickReplyBox
						message={message}
						recipientEmail={getMessageAddress(message.from) || ""}
						recipientName={getMessageName(message.from)}
						onExpandToFullEditor={() => {
							setShowEditor(true);
							setShowEditorMode("reply");
						}}
						onSend={async (content) => {
							// For now, expand to full editor with content
							// TODO: Implement direct quick send
							setShowEditor(true);
							setShowEditorMode("reply");
						}}
					/>

					{/* Forward button - separate since quick reply doesn't support forward */}
