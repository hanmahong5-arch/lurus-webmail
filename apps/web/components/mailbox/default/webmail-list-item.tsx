...
import { Mail, MailOpen, Paperclip, Trash2, GripVertical } from "lucide-react";
...
export default function WebmailListItem({
	...
	// Prefetch thread content on hover for faster navigation
	...
	const { attributes, listeners, setNodeRef, transform, isDragging } = useDraggable({
		id: mailboxThreadItem.threadId,
		data: {
			threadId: mailboxThreadItem.threadId,
			subject: mailboxThreadItem.subject,
			selectedCount: 1,
	...
	// Handlers for preview popover actions
	const handlePreviewReply = () => {
		openThread();
		// Reply will be handled on the thread page
	};

	const handlePreviewForward = () => {
		openThread();
		// Forward will be handled on the thread page
	};
	...
	return (
		...
		>
			<HoverCard.Target>
				<li
					...
					className={[
						...
						"grid-cols-[auto_auto_auto_20rem_minmax(10rem,2fr)_auto]",
			...
			<HoverCard.Dropdown>
				<EmailPreviewPopover
					threadItem={mailboxThreadItem}
					onReply={handlePreviewReply}
