...
// Helper to handle Promise.allSettled results and log failures
async function settledWithLogging<T>(
	promises: Promise<T>[],
	...
	if (failures.length > 0) {
		console.error(
			`[${operationName}] ${failures.length}/${results.length} operations failed:`,
